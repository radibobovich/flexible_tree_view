...
class FlexibleTreeView<T> extends StatefulWidget {
  ...
  // addition to be able to control the scroll position from outside
...
class RecommendNodeWidget<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    while (showLines && parent != null) {
      ...
      if (true || parent.children.length > 1) {
        verticalLines.add(
          Positioned(
            top: 0,
            bottom: 0,
            left: parent.depth * indent + 6,
      ...
      child: Stack(
        children: [
          Padding(
            ...
            // padding: node.depth <= 16
            //     ? EdgeInsets.only(left: node.depth * indent)
            //     : node.depth <= 32
            //         ? EdgeInsets.only(left: (node.depth - 16) * indent)
            //         : EdgeInsets.only(left: (node.depth - 32) * indent),

            // reset indent every 16 posts
            padding: (node.depth ~/ 17 == 0 || scrollable)
                ? EdgeInsets.only(left: node.depth * indent)
                : EdgeInsets.only(
                    left: (node.depth - (node.depth ~/ 17) * 17) * indent),
            child: SizedBox(
              width: nodeWidth,
          ...
          if (showLines &&
              node.depth > 0 &&
              (true || (node.parent?.children.length ?? 0) > 1))
            Positioned(
              top: 0,
              bottom: 0,
              left: (node.depth - 1) * indent + 6,
              width: indent,
              child: const Divider(
                //color: lineColor ?? Colors.black54,
                thickness: 1,
...
class TreeNode<T> with ChangeNotifier {
  ...
  /// Creates a global key for each FlexibleTreeView widget that uses this node.
  /// Used to avoid creating multiple PostWidget's with the same GlobalKey,
  /// for example, when user has a thread tab and a branch tab both containing
  /// the same post.
  /// In the example it creates separate set of keys for thread and branch tabs.
  /// [id] stands for id of the thread or branch node that contains the node.
  GlobalKey getGlobalKey(int id) {
    GlobalKey? key = _globalKeys[id];
    if (key == null) {
      key = GlobalKey();
      _globalKeys[id] = key;
    }
  ...
  TreeNode<T> copyWith(
    ...
    _children?.forEach((element) {
      copyNode.children.add(element.copyWith(
          parent: copyNode,
          newKey: newKey,
          key: newKey ? UniqueKey().toString() : null));
    });
    return copyNode;
  }
}
